# Gate.io Listing Application — Temptation Token ($TTS)
*Submit at: gate.io/en/listingrequest or email list@gate.io*

## Token Details

| Field | Value |
|-------|-------|
| Token Name | Temptation Token |
| Token Symbol | TTS |
| Blockchain | Base (Ethereum L2, Chain ID 8453) |
| Token Standard | ERC-20 |
| Contract Address | 0x5570eA97d53A53170e973894A9Fa7feb5785d3b9 |
| Total Supply | 69,000,000,000 |
| Decimals | 18 |
| Website | https://temptationtoken.io |
| App | https://app.temptationtoken.io |

## Project Description

Temptation Token ($TTS) is a weekly vote-to-earn game on Base blockchain. Users vote $TTS on competing profiles each week; the top voter on the winning profile wins 35% of the prize pool automatically at round close. Prize distribution: 35% to the top voter, 35% to the winning profile, 10% to Polaris Project charity, and 20% to Blockchain Entertainment LLC. When a profile was submitted through a club partner, the split adjusts to 35/35/10/10/10 — the club receives 10% and the house receives 10%. Chainlink VRF ensures provably fair winner selection.

Staking system: Bronze (8% APR, 1.1x) through VIP (45% APR, 3x). Fixed supply, no inflation. Audited by Solidproof.

## Exchange Listing Details

| Field | Value |
|-------|-------|
| Requested Trading Pair | TTS/USDT |
| Current Market | Uniswap V2 Base (0x77Fe18…721ce68) |
| Desired Listing Date | ASAP |
| Market Maker | Available upon request |

## Security Documentation

| Item | Status | Link |
|------|--------|------|
| Smart Contract Audit | ✅ Solidproof — zero critical | temptationtoken.io/audit |
| LP Lock | ✅ 12 months | team.finance |
| Verified Contract | ✅ BaseScan | basescan.org/address/0x5570eA... |
| Chainlink VRF | ✅ Active on Base | vrf.chain.link/base |

## Social Links

- Twitter: https://twitter.com/TemptationToken
- Telegram: https://t.me/temptationtoken
- Instagram: https://instagram.com/temptationtoken

## Contact

Blockchain Entertainment LLC
support@temptationtoken.io
Jim Goetz, Founder

## Gate.io Submission Steps

1. Go to gate.io/en/listingrequest
2. Fill out all fields using data above
3. Attach: audit PDF, LP lock proof screenshot, contract verification screenshot
4. Provide community metrics (Telegram members, Twitter followers)
5. Gate.io will respond within 7-14 business days with requirements
6. Gate.io typically requires: market maker arrangement, listing fee (variable), and community milestones
