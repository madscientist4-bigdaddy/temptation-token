# CoinMarketCap Listing Application — Temptation Token ($TTS)
*Submit at: coinmarketcap.com/request → "Add a Cryptocurrency"*

## Required Information

**Project Name:** Temptation Token
**Token Symbol:** TTS
**Token Type:** ERC-20
**Network:** Base (Ethereum Layer 2, Chain ID 8453)
**Contract Address:** 0x5570eA97d53A53170e973894A9Fa7feb5785d3b9
**Total Supply:** 69,000,000,000 (69 billion)
**Circulating Supply:** (check live on BaseScan at time of submission)
**Decimals:** 18

## Description (for CMC listing — 150 words)

Temptation Token ($TTS) is the first vote-to-earn gaming token on Base blockchain. Weekly voting rounds open Monday 12:00 AM EDT and close Sunday 11:59 PM EDT. Users vote $TTS on competing profiles — the top voter on the winning profile automatically receives 35% of the weekly prize pool. The winning profile receives 35%. Prize distribution: 35% to the top voter, 35% to the winning profile, 10% to Polaris Project charity, and 20% to Blockchain Entertainment LLC. When a profile was submitted through a club partner, the split adjusts to 35/35/10/10/10 — the club receives 10% and the house receives 10%. Settlement is executed by Chainlink VRF, making outcomes provably fair and immutable.

TTS is audited by Solidproof (zero critical findings), with liquidity locked 12 months on Team.Finance and treasury held in a Gnosis Safe 2/2 multisig. The staking system offers up to 45% APR with vote multipliers up to 3x. Minted as a fixed supply of 69 billion tokens with a 1% transfer tax funding the prize ecosystem.

**Token Utility:** Voting in weekly rounds, staking for APR + vote boosts, profile submissions, prize payouts.

## Market Information

**Primary Exchange:** Uniswap V2 (Base)
**Pool/Pair Address:** 0x77Fe188379BEaAd3BCFb26c965c812CEa721ce68 (TTS/WETH)
**Trading Volume:** (provide live 24h volume from DexScreener at time of submission)

## Links

- Website: https://temptationtoken.io
- Explorer: https://basescan.org/address/0x5570eA97d53A53170e973894A9Fa7feb5785d3b9
- App: https://app.temptationtoken.io
- Twitter: https://twitter.com/TemptationToken
- Telegram: https://t.me/temptationtoken
- Audit: https://app.solidproof.io/projects/temptation-token

## Security Documentation

- Audit firm: Solidproof (app.solidproof.io/projects/temptation-token)
- Audit result: Complete — zero critical findings
- Audit PDF: https://app.solidproof.io/projects/temptation-token
- LP lock: ✅ Team.Finance — 231.3 LP tokens locked 12 months (until May 5 2027). TX: 0xd98b2bb4c3cfbc57e988ab9898843fab4df20fd87dc216aa1b588f65576779da
- Multisig: Gnosis Safe — 0xeFb59d88179edC49bDA60B43249722Ea0DE6fB86

## Submission Checklist

- [ ] Create CMC account at pro.coinmarketcap.com
- [ ] Go to "Request Form" → "Add a Cryptocurrency"
- [ ] Fill all fields using data above
- [ ] Upload logo (300x300px, white or transparent background, PNG)
- [ ] Include audit PDF link
- [ ] Include LP lock proof URL from Team.Finance
- [ ] Submit and wait 7-14 business days
- [ ] Check status at pro.coinmarketcap.com/request-form-status
