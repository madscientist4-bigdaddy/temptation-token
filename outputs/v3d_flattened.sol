...
// contracts/TTSVotingV3d.sol

// TTSVotingV3d — Upgraded from V3c
//
// Changes from V3c:
//   OWNERSHIP   adminTransferOwnership(address) added — lets the admin (Bank wallet)
//               reclaim or reassign the owner role (normally held by TTSKeeper3) in
//               an emergency, without needing owner signature.
//
// Everything else is identical to V3c:
//   MULTIPLIERS   tier 3 (Diamond): 2.0x, tier 4 (VIP): 3.0x, tier 5 removed
//   NFT MINTS     3 NFTs per settlement: winning profile, top voter, houseWallet archive
//   VOTE CAPS     Per-tier single-tx cap: Unstaked 500, Bronze 1000, Silver 2500,
//                 Gold 5000, Diamond 15000, VIP unlimited
//   PRIZE SPLIT   No club: 35/35/10/20. With club: 35/35/10/10/10.
//
// Storage layout: slots 0–12 identical to V3b/V3c (no new state variables)
//
// Compile: Solidity 0.8.20, optimizer 200, evmVersion paris, viaIR=false.
//
// Deployment sequence: see outputs/deploy_v3d.mjs
...
contract TTSVotingV3d is Ownable, VRFConsumerBaseV2Plus {
    ...
    // V3d addition: lets admin reclaim/reassign the owner (normally the Keeper).
    function adminTransferOwnership(address to) external onlyAdmin {
    ...
    // Per-tier single-tx vote caps (tier 4 VIP = unlimited)
    ...
    // -------------------------------------------------------------------------
    // Round management (owner = TTSKeeper3)
    // -------------------------------------------------------------------------
    ...
    function tierVoteCap(address voter) public view returns (uint256) {
        try stakingContract.getStakingTier(voter) returns (uint256 tier) {
            ...
            return type(uint256).max; // VIP: no cap
        } catch {}
        return VOTE_CAP_UNSTAKED;
    }
